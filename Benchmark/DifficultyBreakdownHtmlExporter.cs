...
namespace Benchmark
{
    ...
    public static class DifficultyBreakdownHtmlExporter
    {
        ...
        {
            var redSwings = ratings.SwingData.Where(x => x.Start.Type == 0).ToList();
            var blueSwings = ratings.SwingData.Where(x => x.Start.Type == 1).ToList();
        ...
        private static void GeneratePerSwingTable(StringBuilder html, List<SwingData> swingData, float bpm)
        {
            ...
            for (int i = 0; i < swingData.Count; i++)
            {
                ...
                string handClass = swing.Start.Type == 0 ? "red" : "blue";
