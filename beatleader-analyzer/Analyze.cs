...
namespace beatleader_analyzer
{
    public class Analyze
    {
        ...
        public List<Ratings> GetRatingWithCurveDebug(BeatmapV3 beatmap, string characteristic, float timescale = 1, float njsMult = 1, bool strictAngles = false)
        {
            List<Ratings> ratings = [];
            var data = beatmap.Info._difficultyBeatmapSets.FirstOrDefault(x => x._beatmapCharacteristicName == characteristic);
            ...
            try
            {
                foreach (var difficulty in beatmap.Difficulties.Where(x => x.Characteristic == characteristic))
                {
                    if (difficulty.Data.Notes.Count >= 20)
                    {
                        Ratings rating = Analyzer.BeatmapScanner.BeatmapScanner.AnalyzerWithCurveDebug(difficulty.Data.Notes, difficulty.Data.Chains, difficulty.Data.Bombs, difficulty.Data.Walls, beatmap.Info._beatsPerMinute, timescale, njsMult, strictAngles);
                        rating.Characteristic = difficulty.Characteristic;
                        rating.Difficulty = difficulty.Difficulty;
                        ratings.Add(rating);
                    }
