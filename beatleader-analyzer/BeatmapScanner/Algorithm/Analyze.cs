...
namespace Analyzer.BeatmapScanner.Algorithm
{
    ...
    internal class Analyze
    {
        ...
        public static Ratings UseLackWizAlgorithmWithCurveDebug(List<Cube> red, List<Cube> blue, float bpm, List<Wall> walls = null, List<Bomb> bombs = null, bool strictAngles = false)
        {
            List<SwingData> redSwingData = [];
            List<SwingData> blueSwingData = [];
            List<SwingData> combinedSwingData = [];
            ...
            if (red.Count > 2)
            {
                ...
                if (redSwingData.Count > 0)
                {
                    SwingCurve.CalcWithDebug(redSwingData, false);
                    combinedSwingData.AddRange(redSwingData);
                }
            }
            ...
            if (blue.Count > 2)
            {
                ...
                if (blueSwingData.Count > 0)
                {
                    SwingCurve.CalcWithDebug(blueSwingData, true);
                    combinedSwingData.AddRange(blueSwingData);
                }
            }
